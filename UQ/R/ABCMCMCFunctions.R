...
#' @param startPar starting vaue for the parameter vector
#' @param parIdx NEEDS CLARIFICATION
#' @param parDefVal NEEDS CLARIFICATION
...
#' @param copula An RVineMatrix() asreturned by RVineStructureSelect
...
#' @param Z (CDE) cummulative density estimates of U
...
#' @return probability denisty function value
...
#' @param modelName (characters), this will be used to find the model file and the functions in that file
#' @param parIdx remapping index set for passing parameters to the model
#' @param parDefVal default values for the parameters (for cases in which some never change)
#' @param curPar current parameter values (as ABC samples them)
#' @param canPar candidate parameter values (for MCMC)
#' @param curDelta current distance between data and simulation, if this is above threshold (delta) any run that is closer to the data than this is accepted as the new current state (for the MCMC variables).
#' @param curPrior current Prior values given curPar
#' @param delta distance thershold for ABC
...
#' @param cupula as returned by copula estimators
#' @param ll lower limit of parameters
#' @param ul upper limit of parameters
#' @param getScore a scoring function
#' @param environment "C" selects GSL solvers, "R" selects the deSolve as backend
