...
vignette: >
  %\VignetteIndexEntry{uqsa-AKAP79}
...
We use `icpm-kth/SBtabVFGEN` to read the model files, `icpm-kth/rgsl` as an
interface to `gsl_odeiv2` solvers (for initial value problems):
...
variables `parABC` into something that the model will accept. The plan
