...
This is the created file that will be used in simulations:
...
evolution in time of the concentrations of compounds in the
...
(specified through the input argument `modelName`) given the
experimental conditions saved in `ex`.

```{r}
simulate <- simulator.c(ex,modelName)
simulate_noise <- simulator.c(ex,modelName, noise = TRUE)
```
