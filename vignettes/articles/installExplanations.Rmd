...
gcc only through xcode is despicable as they have no right to
gate-keep any of these.
...
For this to have the desired effect, `~/.local/bin` needs to be in
your [PATH](articles/path.html) environment variable.

If it is, then `pkg-config` becomes available from any directory, even
