...
saveRDS(sb,file="AKAP79tc-sb.RDS")
saveRDS(ex,file="AKAP79tc-ex.RDS")
```
...
start_time=$((`date +%s`))
date
mpirun -H localhost:$N ./pt-smmala-akap79.R $((2**16))
date
end_time=$((`date +%s`))
...
```{r uwerr}
Sample <- readRDS(file="AKAP79-temperature-ordered-smmala-sample-2-for-rank-0.RDS")
n <- NROW(Sample)
