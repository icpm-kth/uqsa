...
```{r, label="test-order-1", eval=FALSE, include=FALSE}
cat("test",file="test.txt")
```

```{sh, label="test-order-2", eval=FALSE, include=FALSE}
cat test.txt
echo "MODIFICATION" >> test.txt
```

```{r, label="test-ordre-3", eval=FALSE, include=FALSE}
l <- readLines("test.txt")
cat(l,sep="\n")
```
...
```{sh MPI, eval=FALSE}
N=4 # by default
[ -e '/proc/cpuinfo' ] && N=$((`grep -c processor /proc/cpuinfo`))
echo "We will use $N cores"

date
mpirun -H localhost:$N ./pt-mh-akap79.R > /dev/null 2>&1
date
```
...
```{r uwerr}
Sample <- readRDS(file="./AKAP79-parameter-sample-for-rank-0.RDS")
n <- NROW(Sample)
