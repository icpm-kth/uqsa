...
distanceMeasure <- function(funcSim, dataExpr, dataErr = NA){
  ...
  distance <- mean(((funcSim['AKAR4pOUT',]-dataExpr['AKAR4pOUT',])/max(dataExpr['AKAR4pOUT',]))^2,na.rm=TRUE)
  return(distance)
}
```
...
for (i in seq(length(chunks))){
	...
	prior <- rprior(1000)
	posterior <- ABCSMC(Obj,t(prior),dprior=dprior,messages=TRUE)
	if (i < length(chunks)){
...
if (require(hadron)){
    ...
    ac <- acf(mcmc$scores,lag.max=1000)
    l <- which(A$acf>0)
    tau <- sum(A$acf[l])
    i <- seq(1,NROW(mcmc$draws),by=tau)
}
