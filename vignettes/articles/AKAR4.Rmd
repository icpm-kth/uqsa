...
The R file we source includes a variable called `model`, it is
a list of functions, one of which is called `model$par()` and returns
...
The file ending in `gvf.c` contains functions used for
simulations. The functions in `.R` can be used more freely in R, and
are functionally identical (they describe the same model).
...
consists of the biological model parameters (which are the same fro
all experiments), and input parameters (which together with initial
conditions distinguish the experimental setups). These two different
kinds of parameters are concatenated into one big numeric vector
inside of the solver. The model function `AKAR4_default()` returns the
default values for `p`.

In SBtab, we make a clear distinction between the two: biological
paramteters go into the `SBtab$Parameter` table, while input
parameters can be found in `SBtab$Input`. The input values are adjusted in
the rows of the `SBtab$Experiments` table.

We use the SBtab information to determine the length of the biological parameters `numPar` and shorten the value of  default
parameter values in `SBtab` to start sampling:

```{r, "starting value"}
numPar <- nrow(SBtab$Parameter)
