...
The `pkgconf` on alpine already provides the `pkg-config`
command, no symlink necessary:
...
systems (it is impossible for to list all of them). 
...
science-gods have mercy), then you need a package manager that
...
gcc only through xcode is despicable.
...
not something that has anything to do with us. Everyone who wants to
use `pkg-config` via pkgconf on macOS needs to consider these things.
...
isn't).  If `which pkgconf` prints a valid path (it should), then make
a symlink to it:
...
Look at the directories listed in the `PATH` environment variable,
find one where you can add files (for me this is `~/.local/bin`) and
make a symbolic link there using `ln`.
