...
- write the model in vfgen's `vf` format
	...
	+ [`ode.sh`](icpm-kth/RPN-derivative) does not use an xml parser since there is no mathml
...
Let's take this [model of a pendulum](https://warrenweckesser.github.io/vfgen/menu_xpp.html) as an
...
EOF
) > /dev/shm/pendulum.ode
```

We will use this model, and convert it to the text format that
`ode.sh` can read using standard command line tools:
...
It runs whenever the model-file ends in `ode`:
