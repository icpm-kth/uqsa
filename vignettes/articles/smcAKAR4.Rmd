...
cat(difftime(t_final,t_initial))
```
...
if (requireNamespace(hexbin)){
	hexbin::hexplom(posterior$draws, xbins = 16)
} else {
...
```{r trajectories, fig.height=20, out.width="100%"}
Z <- posterior$draws
y <- s(t(Z)) # using the stochastic simulator one final time
par(3,1)
for (i in seq_along(ex)){
	tm <- ex[[i]]$outputTimes
	plot(as.errors(tm),ex[[i]]$data,xlab="time",ylab="AKAR4p",main=names(ex)[i])
	matplot(tm,y[[i]]$func,type='s',col=rgb(0,0,1,0.1))
}
```
