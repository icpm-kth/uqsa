...
# Function that, given the state of the system, outputs the value(s) measured in the experiment
# E.g., the AKAR4 model has 4 states (AKAR4, AKAR4_C, AKAR4p, and C), 
# but in the experiments only one of these states (AKAR4p) is measured
outputFunction <- function(state){model$func(t=0, state=state, parameters = par_val)}
...
```{r makeObjective}
objectiveFunction <- makeObjectiveSSA(experiments = experiments, model.tab = SBtab, parNames = par_names, outputFunction = outputFunction, distance = distance, parMap = parMap, reactions = reactions, nStochSim = 3)
```
...
#clusterExport(cl, c("objectiveFunction", "startPar", "ns", "Sigma", "delta", "dprior"))
