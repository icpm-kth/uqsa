...
- mathematical expressions can be inequalities, e.g. `a < b` which
  evaluates to `0` or `1` in C and is as valid a math expression as
  `a + b` 
...
Ginac cannot parse vf files with inequality expressions.

Assuming that you followed all [installation instructions](articles/installation.html):

```{sh, eval=FALSE}
ode -R --maxima ${MODEL:-myModel}.vf > ${MODEL:-myModel}.R 
```
