...
The included example model called _CaMKII_ illustrates an interesting case:
...
arbitrarily set a time between input doses and simulate all points in
...
like a staircase at all. This may or may not be desirable (depends on
whether the original experiment was in steady state).

Because most data-structures are just plain R data types, it is easy
to modify experiments and experiment with the simulations. Here we
make the entire time-course longer
...
Because we changed the experimental setup, we recreate the solver function:
```{r solver-2}
s <- simulator.c(ex, o, parMap=logParMap)
y <- s(p) # new solution
```
...
```{r, label="longer-simulation-Bradshaw", echo=FALSE, out.width="100%", fig.height=7, fig.width=5}
oldpar <- par(mfrow=c(3,2),bty="n")
for (i in seq_along(ex)){
	j  <- rownames(m$Output) %in% colnames(ex[[i]]$measurements)
	t_siml <-ex[[i]]$outputTimes
...
system to go into steady state quickly enough, even if we increase
the time between the transformation events more. On the other hand,
the original data was measured with $600\,\text{s}$ equilibration time, not in
verified steady state.

## Asymmetry

The experiments for this model are different in complexity. How many
seconds the solver needs to calculate a simulated trajectory depends
...
solver: no solution from the chosen solver means that the likelihood
...
![Stretched time line for Experiment 1 (event-schedule and measurement times).](./CaMKIIs-integrators-stretched-time-1.png)
...
final point needs to equilibrate individually.  We plot the dose response,
by collecting it from the solution:
