...
```{r test}
s <- simulator.c(ex,modelName,parMap=log10ParMap)
...
f_ <- y[[1]]$func[1,,1]
par(bty="n")
plot(t_,d_,type="p",main=names(ex)[1],xlab="time",ylab=ylab)
...
The Metropolis Hastings Algorithm does not require any model functions (The R variant of the generated code), so we set model to `NULL`:

```{r likelihood}
llf <- logLikelihoodFunc(ex)
metropolis_hastings <- mcmcUpdate(s,ex,model=NULL,logLikelihood=llf,dprior=dprior)
...
if (require(hadron)){
    ac <- hadron::uwerr(data=L_,nrep=rep(10000,8),pl=TRUE)
}
