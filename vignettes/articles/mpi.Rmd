...
This is different from the `parLapply` paradigm, where work is
...
Locally you can test the MPI script using the `mpirun` command:

```{sh, eval=FALSE, label="mpirun"}
mpirun -H localhost:16 -N 8 ./AKAP79.R 5000
```

For this to work, `AKAP79.R` has to start with the line:
...
mpirun -H localhost:16 -N 8 Rscript ./AKAP79.R 5000
```

On the cluster the `-H` option is not necessary, because a queue
system like slurm will determine which machines to use.

For this particular script the argument `5000` is the sample size and
`-N 8` means that we launch 8 MPI workers per node (node means the
same as machine or host). We have one node (localhost), with 16 slots
to run workers in (because we assume 16 cores).

On a cluster, we can have many more chains.


# MPI Script Template

This is an example that will sample a model using the Metropolis
algorithm, using `pbdMPI` and the parallel tempering technique.

```{r, eval=FALSE, label="mpi-pt-sampling.R"}
#!/usr/bin/env Rscript

## ^^^ the above has to be on line 1, 
...
## ----ConservationLaws----------------------------------------------------------
if (file.exists("../ConservationLaws.RData")){
	load("../ConservationLaws.RData")
	if (any(is.na(ConLaw))) {
	  ConLaw <- NULL
	}
} else {
...
## ----range--------------------------------------------------------------------
stdv <- head(sb$Parameter[["!Std"]],length(parMCMC))              # this as well
...
saveRDS(Copula,file=sprintf("Copula-for-beta-%i-%g.RDS",r,-100*log10(beta)))
saveRDS(X,file="parameter-sample-for-beta-%i-%g.RDS",r,-100*log10(beta))
