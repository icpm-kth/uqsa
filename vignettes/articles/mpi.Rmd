...
If the likelihood function is not derived from a Gaussian, then the
...
a very big sample (millions of sampled points), we could use the real
...
initialize a Markov chain with good values:
...
it is beneficial (according to the specific rules of parallel
...
chains. At this point, it becomes beneficial to do the simulations on
...
also start 24 chains distributed across 3 nodes.
