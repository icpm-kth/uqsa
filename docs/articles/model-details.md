...
label the first column with `id` (but it can have any name).

They can be imported into R using the command
[`uqsa::model_from_tsv()`](https://icpm-kth.github.io/uqsa/reference/model_from_tsv.md):
...
ex <- experiments(m,o)    # simulation experiments
```

To write a model file, look at one of the example TSV file collections
and copy their structure.

The model tables follow these guidelines:

- A list of quantities, like parameters have these columns (spaces are
  allowed):
  - id (first column, any heading)
  - value (a default value, possibly subject to estimation)
  - unit
  - scale (optional, assumed to be linear)
  - prior distribution (optional, assumed Gaussian)
  - mean (optional parameter for prior distribution)
  - median (optional)
  - sd, stdv, standard deviation, standard error (all optional)
- A list of mathematical expressions can use the term `formula` instead
  of `value`:
  - id
  - formula
  - unit
...
- Data tables need to specifiy values using a notation that keeps values
  ...
  - `1.234(21)`
  - `1.5(1)e+4`
- The tables are found by name, the names our import scripts will be
  looking for are:
  - Constant.tsv
  - Parameter.tsv
  - Input.tsv
...
This package will assume that the directory/folder where the TSV files
reside in is the name of the model. This is an example folder structure:
...
Reacting compounds, or molecular species. These will be turned into
state variables.
...
These are parameters, that will be subject to optimization or sampling.
The values are assumed to be crude estimates.
...
Each row describes an observable, and how to measure it with this model
...
contains a formula that calculates the new value. The id can be used in
event schedules to reference this transformation.
...
A Dose Response table is a more convenient way of describing a series of
experiments, each with its own, very \_similar\_ conditions, and exactly
...
The event schedule we used here is the same as in the section about
events:
